...
const gradingTools = {
  ...
  add_annotation: { description: 'Add, update or remove one annotation on a scan (badge/comment/circle/cross/underline/pen). Pass annotationId with remove:true to delete, or annotationId alone to update fields. bbox must be a flat 4-element array [x, y, width, height] in original-image pixels (numeric strings accepted).', fields: { annotationId: { type: 'string' }, remove: { type: 'boolean' }, kind: { type: 'string', enum: ['badge', 'comment', 'circle', 'cross', 'underline', 'pen'] }, bbox: { type: 'array', items: {}, minItems: 4, maxItems: 4, description: '[x, y, width, height] in original-image pixels; numeric strings accepted.' }, content: { type: 'string', description: 'Annotation text (score label, comment, etc.).' }, color: { type: 'string' }, width: { type: 'number' }, fontFamily: { type: 'string' }, solid: { type: 'boolean' }, points: { type: 'array', items: {}, description: 'Freehand pen point list [x1,y1,x2,y2,...] as flat numbers.' } }, required: ['scanId'] },
  add_annotations: { description: 'Add MULTIPLE annotations to a scan in one call (agent loop: batch-add, then grading_preview to check, then fix by annotationId). Each entry needs kind and a flat 4-number bbox [x,y,w,h].', fields: { annotations: { type: 'array', items: { type: 'object', properties: { kind: { type: 'string', enum: ['badge', 'comment', 'circle', 'cross', 'underline', 'pen'] }, bbox: { type: 'array', items: {}, minItems: 4, maxItems: 4 }, content: { type: 'string' }, color: { type: 'string' }, width: { type: 'number' }, fontFamily: { type: 'string' }, solid: { type: 'boolean' }, points: { type: 'array', items: {} } }, required: ['kind', 'bbox'] } } }, required: ['scanId', 'annotations'] },
