...
describe('hermes-web-ui MCP server', () => {
  ...
  it('exposes a public Web UI API requester tool', async () => {
    ...
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_get')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_lines')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_view_image')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_preview')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_add_annotation')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_add_annotations')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'grading_rotate')).toBe(true)
  ...
  it('filters exposed tools by MCP toolset argument', async () => {
    ...
    expect(catalog).toMatchObject({ toolset: 'devices', operation_count: 18 })
    expect(catalog.operations.map((tool: any) => tool.name)).toEqual(expect.arrayContaining([
      ...
      'hermes_studio_usb_devices_list',
      'hermes_studio_usb_file_read',
    ]))
    ...
    const legacyUsbDevicesCall = await waitForRpc(responses, 7)
    ...
    const usbReadCall = await waitForRpc(responses, 8)
