...
describe('TRPG highlight', () => {
  it('normalizes names and emits explicit actions without leaking character card secrets', async () => {
    ...
    const d = deps(output)
    const result = await generateHighlight(parseInput(input), d)
  ...
  it('rejects unknown characters and fabricated quotes', async () => {
    for (const action of [{ ...output.actions[0], characterId: 'unknown' }, { ...output.actions[0], evidence: '击败巨龙' }]) {
      await expect(generateHighlight(parseInput(input), deps({ ...output, actions: [action] }))).rejects.toThrow('invalid_output')
    }
  })
  it('reports no highlight and unavailable model distinctly', async () => {
    await expect(generateHighlight(parseInput(input), deps(null))).rejects.toThrow('no_highlight')
    await expect(generateHighlight(parseInput(input), { loadConfig: async () => null })).rejects.toThrow('llm_not_configured')
  })
  it('does not expose provider error bodies', async () => {
    await expect(generateHighlight(parseInput(input), { loadConfig, fetchImpl: vi.fn(async () => new Response('secret', { status: 500 })) as unknown as typeof fetch })).rejects.toThrow('generation_failed')
  })
})
